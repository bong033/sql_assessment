...
-- ****************************************************************
SELECT COUNT(email_domain) AS `Count of email domain`, email_domain FROM users 
WHERE email_domain LIKE "%.edu"
GROUP BY email_domain
ORDER BY `Count of email domain` DESC
LIMIT 25;
...
-- ****************************************************************
SELECT HOUR(STR_TO_DATE(sign_up_at, '%Y-%m-%d %H:%i:%s')) AS hour, COUNT(*) AS sign_up_at_count
FROM users
GROUP BY hour
ORDER BY hour;
